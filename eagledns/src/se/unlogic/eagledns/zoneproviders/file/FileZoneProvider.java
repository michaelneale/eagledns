...
import se.unlogic.eagledns.ZoneProviderUpdatable.ZoneChangeCallback;
 ...
 *
 */
public class FileZoneProvider implements ZoneProvider, ZoneProviderUpdatable {
	...
	public void init(String name) {
		...
		watcher.schedule(new TimerTask() {
			@Override
			public void run() {
				if (changeCallback != null && hasDirectoryChanged())	changeCallback.zoneDataChanged();
			}
		}, 5000, 4000);
	}
	...
	private boolean hasDirectoryChanged() {
		...
		if (files.length != lastFileList.size()) return true;
		for (File f : folder.listFiles()) {
			if (!lastFileList.containsKey(f.getName())) return true;
			if (f.lastModified() > lastFileList.get(f.getName())) return true;
		}
